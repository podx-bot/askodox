...
class Database:
    ...
    def execute(self, query: str, parameters: Iterable = ()) -> sqlite3.Cursor:
        with self._lock:
            cursor = self.connection.execute(query, tuple(parameters))
            self.connection.commit()
            return cursor

    def fetchone(self, query: str, parameters: Iterable = ()) -> Optional[sqlite3.Row]:
        with self._lock:
            return self.connection.execute(query, tuple(parameters)).fetchone()

    def fetchall(self, query: str, parameters: Iterable = ()) -> list[sqlite3.Row]:
        with self._lock:
    ...
    def create_tables(self) -> None:
        ...
        migrations = {
            ...
            "location_address": "TEXT", "location_updated_at": "TEXT", "role": "TEXT",
            "job_category": "TEXT", "experience": "TEXT", "availability": "TEXT",
            "worker_registration_complete": "INTEGER NOT NULL DEFAULT 0"
        }
        ...
        """)
        self._add_column_if_missing("employer_jobs", "required_workers", "INTEGER NOT NULL DEFAULT 1")
        self._add_column_if_missing("employer_jobs", "employer_contact", "TEXT")
        self.execute("CREATE INDEX IF NOT EXISTS idx_employer_jobs_mobile_status ON employer_jobs(employer_mobile, status)")
        self.execute("""
        ...
        """)
        self.execute("CREATE INDEX IF NOT EXISTS idx_job_assignments_job_status ON job_assignments(employer_job_id, status)")
        self.execute("CREATE INDEX IF NOT EXISTS idx_job_assignments_worker_status ON job_assignments(worker_mobile, status)")
        self.execute("""
        ...
        """)
        self.execute("CREATE INDEX IF NOT EXISTS idx_appointment_requests_customer_status ON appointment_requests(customer_mobile, status)")
        self.execute("""
