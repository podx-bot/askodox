...
### 2026-09-16 (round 14) — URGENT hotfix: round 13 broke the app's main "publish a request / accept a match" flow for every real signed-in user

While starting to investigate `debug.py` (the recommended next round per round 13's own tracker entry), a check of what actually calls `POST /deals` and `POST /deals/{id}/accept-match` in Flutter turned up a serious, already-live regression from round 13, unrelated to `debug.py`.

**What went wrong:** round 13 correctly required a real signed session token on both of those backend endpoints (`universal_deals.py`). But those exact two endpoints are not only reachable through the deal-inbox screens fixed in round 13 (`deal_screens.dart`) -- they are the same endpoints the app's **main matching flow** uses: `lib/features/matching/data/universal_match_repository.dart` (`ApiUniversalMatchRepository`, used by `universal_match_screen.dart` and `home_screen.dart` -- the "publish a buy/sell/service request, then see and accept matches" journey) calls `POST /deals` from `createAndMatch()` and `POST /deals/{id}/accept-match` from `acceptMatch()`. Round 13's own verification pass checked `deal_screens.dart` carefully but did not check whether any *other* Flutter file called the same two backend routes -- it did not, and this file does, and it was never updated to send a token. The result, live in production from the moment round 13 merged: every real signed-in user's attempt to publish a new request or accept a match failed with "Sign in required -- no session token was sent", even though they genuinely were signed in. This is the app's central feature, so this was treated as urgent and fixed immediately, ahead of the previously-planned `debug.py` round.

Fix shipped this round (1 file):
- `lib/features/matching/data/universal_match_repository.dart`: `universalMatchRepositoryProvider` and `ApiUniversalMatchRepository` updated to send the real session token, mirroring `order_repository.dart`'s (round 10), `seller_listing_repository.dart`'s (round 12), and `deal_screens.dart`'s (round 13) identical fix exactly. `_createOptions` and `_matchOptions` changed from `static const` to instance getters carrying `authToken`; a new `_acceptOptions` getter was added since `acceptMatch`'s POST call previously had no `options:` at all. A guest (not signed in) now correctly gets a 401 from these two endpoints instead of silently acting under a throwaway `app-guest-<timestamp>` identity -- the same intentional behavior change every round since round 10 has made, now applied consistently here too.

Verification performed:
- Grepped the whole Flutter codebase for every caller of `/deals`, `/deals/{id}/matches`, and `/deals/{id}/accept-match` to confirm `universal_match_repository.dart` was the *only* other caller besides `deal_screens.dart` -- there is no third file this same regression could be hiding in.
- Hand-traced the modified file for brace/paren/bracket balance (all matched: 157 parens, 53 braces, 20 brackets) and diffed line-by-line against `main` to confirm every change is additive/intentional.
- Confirmed the existing `test/sandbox_match_acceptance_test.dart` (4 constructor call sites) still compiles unchanged: `authToken` was added as an optional, not required, named parameter, and all 4 existing calls only exercise the sandbox path (`local-`/`demo-` prefixed ids), which returns before ever reaching the changed HTTP call -- confirmed by reading `_isSandbox()`'s short-circuit logic directly, not just assumed.
- **Not yet verified by `flutter-ci.yml`/`backend-monorepo-smoke.yml`**, and **not yet exercised end-to-end against a live backend** -- wait for both to go green after this instruction file is applied and pushed, then do the manual end-to-end test described below. Given the severity, this one is worth testing live *before* moving on to anything else.

Known gaps, explicitly not started this round:
- This round is deliberately scoped to only the regression fix -- `debug.py` (the last file with the original zero-proof `_app_user`-style pattern) is unchanged and remains the next recommended round.
- No automated check exists to catch "a backend route's identity requirement changed but not every Flutter caller of that route was updated" -- this round was caught by a manual grep, not a test. Adding a lightweight cross-reference check (e.g., a script that lists every Flutter API call path next to every backend route's auth requirement) is worth considering as its own small future round, given this is the second time in this engagement a Flutter-side gap was found by inspection rather than tooling (the first being round 7's silently-dropped `entities` field).

Next action: apply this instruction file **immediately, ahead of any other pending work** -- production is currently broken for the app's core flow. Wait for `flutter-ci.yml` and `backend-monorepo-smoke.yml` to go green, **create and merge a pull request**, publish a new Android release, update the app, then re-verify by hand as a signed-in (OTP-verified) user: publish a new buy/sell/service request from the home screen and confirm it succeeds and shows matches (or a clear no-match state) instead of "Sign in required"; then accept a real match and confirm that succeeds too. After this is confirmed working live, resume with `debug.py` as previously planned.

### 2026-09-17 (round 15) — Closing out the identity-spoofing audit: debug.py's dead handlers deleted, universal_deals.py's last open authorization gap fixed

With round 14's hotfix confirmed live, this round returns to `debug.py`, the last file flagged since round 10 as still carrying the original zero-proof `_app_user`-style pattern, plus `universal_deals.py`'s `GET /{deal_id}/matches`, flagged as an open gap since round 13 (missing authorization entirely, not a spoofable claim).

**`debug.py` -- self-correction during investigation, not a straight fix:** the plan going in was to authenticate its 4 handlers (`debug_message`, `debug_location`, `debug_inbox`, `debug_match_action`) the same way every other file in this audit was fixed. But a repo-wide search of the Flutter app for any of their 4 request paths (`/debug/message`, `/debug/location`, `/debug/inbox/{user_id}`, `/debug/match-action`) turned up **zero callers** -- nothing in the app has ever sent a request to any of them. The real, live equivalents already exist elsewhere and were already fixed in earlier rounds: `POST /deals` (`universal_deals.py`'s `create_deal`, round 13/14) calls the exact same `container.conversation_service.process(...)` `debug_message` called; `POST /api/in-app/assistant` (`in_app_assistant.py`) is the real classification/reply layer; and `in_app_deal.py`'s `interest_action` (round 13) is the real match-decision handler. This was first reported to the product owner as "safe to simply delete, all 4 are dead code" -- **that was then found to be only half-right**: a follow-up grep against `.github/workflows/` (a check not done before the first recommendation) found that `in-app-e2e-smoke.yml`, a live CI smoke test, directly calls `/debug/message` and `/debug/inbox/{user_id}` via FastAPI's `TestClient`. `/debug/location` and `/debug/match-action` really are fully dead, CI included. This correction was surfaced to the product owner directly, and the explicit instruction given was: rewrite the CI test to exercise the real `/deals` path instead, then delete all 4 handlers -- which is what this round does.

**`universal_deals.py`'s `get_matches`:** unlike every other gap this audit found, this one was missing authorization entirely, not trusting a spoofable claim -- anyone who knew a `deal_id` could see the full list of interested buyers/sellers for it, including counterparty details, with no check that the caller had anything to do with that deal. Fixed the same way `accept_match` was fixed in round 13: the caller's own proven token identity must equal the deal's recorded owner, 403 otherwise (not a 404, to stay consistent with `accept_match`'s existing choice in this same file). No Flutter change was needed for this fix -- `universal_match_repository.dart`'s `_matchOptions` (fixed in round 14) already sends the required token on this exact `GET` call.

Fix shipped this round (3 files):
- `backend/app/api/routes/debug.py`: removed `debug_message`, `debug_location`, `debug_inbox`, `debug_match_action` and their 3 pydantic request models entirely, along with the now-unused `HTTPException`/`BaseModel`/`Field` imports. `_prepare_askodox_app_identity` is kept unchanged -- it is not dead code, `universal_deals.py`'s `create_deal` imports and calls it directly, and it remains essential to the real chat/matching flow. The file's two genuinely-diagnostic endpoints (`/debug/whatsapp-diagnostics`, `/debug/voice-readiness`) are unchanged. A module docstring was added explaining why these 4 handlers were deleted rather than authenticated, so a future reader does not have to rediscover this reasoning from git history.
- `backend/app/api/routes/universal_deals.py`: `get_matches` gained the same `_authenticated_app_user`/deal-owner check `accept_match` already uses (both now share the identical pattern in the same file). No other line in this file changed.
- `.github/workflows/in-app-e2e-smoke.yml`: rewritten to stop depending on the endpoints just deleted. It now mints a real signed session token in-process (`session_tokens.issue_token`, using `app.state.container.settings.session_token_secret` -- the same secret the real backend would sign with, reachable directly since this test already has in-process access to the container) and calls the real `POST /deals` and `GET /deals/{deal_id}/matches` with it. Since `/deals` classifies the message through `container.universal_request_extractor`, which calls the real Gemini API (unavailable in CI -- `GEMINI_API_KEY` is intentionally left blank here, unchanged from before this round), the test stands in for just that one method (`extract`) with a deterministic NEED/PRODUCT result, the same way the previous version of this test never depended on real WhatsApp/Gemini delivery either. Every other line of the real request -- authentication, live-capture persistence, the database row, the HTTP response contract -- runs on the genuine code path. The rewritten test also asserts the two identity checks this round (and round 13's `accept_match`) exist: a `POST /deals` with no token gets 401, a `GET .../matches` with no token gets 401, and a second, differently-authenticated user calling `GET .../matches` on the first user's deal gets 403. The workflow's trigger path list was updated to watch the files that now matter to this test's real subject matter (`universal_deals.py`, `in_app_deal.py`, `session_tokens.py`, `universal_live_capture_service.py`) instead of `debug.py`, which this test no longer touches.

Verification performed:
- `debug.py` and `universal_deals.py` both `ast.parse` clean, and diffed line-by-line against the current `main` branch to confirm the only changes are the 4 handlers' removal (`debug.py`) and the added owner check (`universal_deals.py`) -- nothing else touched.
- Grepped the whole backend `tests/` directory for every removed name (`debug_message`, `debug_location`, `debug_match_action`, `debug_inbox`, `DebugMessageRequest`, `DebugLocationRequest`, `DebugMatchActionRequest`) and confirmed no test file references any of them.
- Confirmed `app_factory.py` only ever imports `debug.py`'s `router` object, never any of the 4 removed function names directly, so removing them cannot break app startup.
- Re-ran this project's existing `run_backend_tests.py`/`pytest_shim.py` harness against every dependency-free backend test file (order repository, product catalog search, seller tiers, seller profile repository, session tokens, active-deal-context resolver, buyer-guide gate, party-conversation-answer, universal-live-capture merge, WhatsApp support-only gate, sponsored items) with both changed files swapped into a real local clone: every test unaffected by this project's two known pre-existing harness gaps (no `pytest.mark.parametrize`/`monkeypatch` support, no `google.genai` package here) still passes.
- Manually exercised the exact fake-extraction payload the rewritten CI test uses against the real, unmodified `UniversalDemandRepository` and `UniversalLiveCaptureService` classes (with hand-written stand-ins only for the pieces genuinely unavailable here -- FastAPI itself, and the real Gemini-backed extractor) plus a real `session_tokens.issue_token`/`verify_token` round trip: confirmed the fake extraction really does create a real, fetchable `universal_need_offer_records` row through the same code the live app uses, the same way a real classification would. Traced `UniversalAwareConversationService.process()`'s full precedence chain by hand (alert/hybrid-support/creator/ledger/ride/event/catering/grocery/seller-escalation/response-commands/product-runtime/image-service, each checked before `live_capture`) to confirm the rewritten test's message ("I need 2kg rice delivered to Vijayawada") matches none of their specific triggers and genuinely falls through to `live_capture`, the same path `/deals` uses in production.
- **Could not run the actual FastAPI app itself** -- fastapi/pydantic remain uninstallable in this audit environment, same limitation as every prior round. This means the rewritten CI workflow's very final wiring (the real HTTP route, with zero interception by an upstream runtime service not checked above) is confirmed by the GitHub Actions run itself once pushed, not by this audit environment. The test is written to fail loudly (an assertion error) rather than silently if that assumption turns out to be wrong.
- **Not yet verified by `flutter-ci.yml`/`backend-monorepo-smoke.yml`/`in-app-e2e-smoke.yml`**, and **not yet exercised end-to-end against a live backend** -- wait for all three to go green after this instruction file is applied and pushed.

Known gaps, explicitly not started this round:
- The mid-round self-correction (recommending outright deletion, then discovering the CI dependency) is recorded here in full rather than smoothed over, matching this engagement's practice in every prior round of documenting a course-correction honestly rather than silently fixing it and presenting only the final answer.
- The `/debug` prefix on `in_app_deal.py`'s real, live endpoints (noted since round 13) is still not renamed -- unchanged this round, for the same reason: renaming a live path is a separate, higher-risk change with no security benefit on its own.
- The "no automated check exists to catch a Flutter caller left behind when a backend route's identity requirement changes" gap noted in round 14 is still open. This round adds a related but different case to watch for -- a *backend* route whose only real remaining caller turned out to be a CI test, not a Flutter file -- which is worth folding into the same future cross-reference-tooling round if one is ever done.
- With this round shipped, every file flagged by the original full-repository identity-spoofing audit (started before round 10) is now either fixed or deleted. No further dedicated identity-fix round is currently known to be needed; the next planned work returns to the 14-point roadmap's remaining phases (3 onward), per the roadmap document itself.
