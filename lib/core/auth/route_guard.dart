...
class RouteGuard {
  ...
  String? redirect(AuthSession session, String location) {
    if (session.status == AuthStatus.suspended || session.status == AuthStatus.deleted) {
      return location == '/account-status' ? null : '/account-status';
    }

    if (session.status == AuthStatus.sessionExpired) {
    ...
    if (location.startsWith('/admin/')) {
      ...
      final adminSubscriptionAllowed =
          ...
          {UserRole.admin, UserRole.superAdmin}.contains(session.role);
      final needed = location.contains('audit')
          ? Permission.viewAuditLog
          : location.contains('subscriptions') && !adminSubscriptionAllowed
              ? Permission.manageSubscriptions
              : null;
      if (needed != null &&
          !(permissions ?? LocalPermissionService()).hasPermission(role, needed)) {
        return '/forbidden';
      }
      if (!{
        UserRole.admin,
        ...
        UserRole.catalogAdmin,
        UserRole.moderationAdmin,
        UserRole.sellerVerificationAdmin,
