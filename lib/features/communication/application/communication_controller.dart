...
class CommunicationState {
  ...
  List<ShopFollow> get followers => followedShops;
...
class CommunicationController extends StateNotifier<CommunicationState> {
  ...
  void respond(String id, {required double price, required int quantity, required DateTime date}) { final request = state.requests.firstWhere((r) => r.id == id); state = state.copyWith(requests: [for (final r in state.requests) if (r.id == id) r.copyWith(status: BuyerRequestStatus.sellerResponded, sellerName: 'ASKODOX Demo Store', offerPrice: price, offerQuantity: quantity, availabilityDate: date) else r], notifications: [AppNotification(id: 'n${DateTime.now().microsecondsSinceEpoch}', audience: Audience.buyer, category: NotificationCategory.request, title: 'Seller response', message: 'ASKODOX Demo Store offered $quantity × ${request.product} at ₹${price.toStringAsFixed(0)}.', createdAt: DateTime.now()), ...state.notifications]); }
