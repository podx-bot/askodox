...
class PriceBenchmarkSection extends ConsumerWidget {
          ...
          data: (b) {
            ...
            return Column(
              children: [
                ...
                ExpansionTile(
                  title: Text(l.priceStandard),
                  children: [
                    ListTile(
                      title: Text(
                        '${l.observedAverage}: ${money(b.overallAverage)}',
                      ),
                      subtitle: Text(
                        '${l.medianPrice}: ${money(b.median)} · '
                        '${l.observations}: ${b.observations}\n'
                        '${l.fairPriceRange}: ${money(b.fairRange.minimum)}–'
                        '${money(b.fairRange.maximum)} · '
                        '${l.volatility}: ${b.volatility.toStringAsFixed(1)}%',
                      ),
                    ),
                  ],
                ),
                  ...
                  children: [
                    for (final days in const [7, 30, 90])
                      Padding(
                        ...
                        child: ChoiceChip(
                          label: Text('$days ${l.days}'),
                          selected:
                              ref.watch(priceHistoryRangeProvider) == days,
                          onSelected: (_) => ref
                              .read(priceHistoryRangeProvider.notifier)
                              .state = days,
                        ),
                      ),
                  ],
  ...
  String _insight(
    ...
    String Function(num) money,
      ...
      switch (i.type) {
        PriceInsightType.localCheaper => l.localCheaperInsight(money(i.amount)),
        PriceInsightType.onlineCheaper =>
          l.onlineCheaperInsight(money(i.amount)),
