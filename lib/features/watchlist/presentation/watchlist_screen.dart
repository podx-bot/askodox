...
class _WatchlistCard extends ConsumerWidget {
        ...
        child: Column(
          children: [
            ListTile(
              ...
              leading: CircleAvatar(radius: 28, child: Text(item.image, style: const TextStyle(fontSize: 28))),
            ...
            SwitchListTile(
              ...
              subtitle: Text(
                '${item.radiusKm.toStringAsFixed(0)} km · ${item.frequency.name}${item.targetPrice == null ? '' : ' · Target ₹${item.targetPrice!.toStringAsFixed(0)}'}',
              ),
              value: item.alertsEnabled,
              onChanged: (value) => ref.read(watchlistProvider.notifier).update(item.copyWith(alertsEnabled: value)),
            ),
            Align(
  ...
  Future<void> _settings(BuildContext context, WidgetRef ref) async {
    var radius = item.radiusKm;
    var frequency = item.frequency;
    final input = TextEditingController(text: item.targetPrice?.toStringAsFixed(0));
            ...
            children: [
              ...
              DropdownButtonFormField<double>(
                ...
                items: [1.0, 2.0, 5.0, 10.0, 25.0]
                    .map((value) => DropdownMenuItem(value: value, child: Text('${value.toInt()} km')))
                    .toList(),
                onChanged: (value) {
              ...
              FilledButton(
                onPressed: () {
                  final target = double.tryParse(input.text);
