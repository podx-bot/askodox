...
class MockProductDiscoveryRepository implements ProductDiscoveryRepository {
  ...
  static const barcodes = {
    '89010001': 'avocado-box',
    '89010002': 'cold-brew',
    '89010003': 'wave-headphones',
    '89010004': 'pocket-charger',
    '89099999': 'coffee',
  };

  static const synonyms = {
  ...
  List<SmartMatch> matchText(String text, List<Product> catalog) {
    ...
    final normalized = synonyms[query] ?? query;
    final result = <SmartMatch>[];

    for (final p in catalog) {
      final name = p.name.toLowerCase();
      final terms = '$name ${p.brand.name.toLowerCase()} '
          '${p.tags.join(' ').toLowerCase()} ${p.categoryId} ${p.subcategoryId}';
      MatchKind? kind;
      double score = 0;
      String reason = '';

      if (synonyms[query] != null && terms.contains(normalized)) {
        kind = MatchKind.synonym;
        score = .86;
        reason = 'Common synonym';
      } else if (name == normalized ||
          p.tags.any((e) => e.toLowerCase() == normalized)) {
        kind = MatchKind.exact;
        score = .98;
        reason = 'Exact catalog match';
      } else if (terms.contains(normalized)) {
        kind = name.contains(normalized) ? MatchKind.exact : MatchKind.brand;
        score = .9;
        reason = 'Name, brand or category match';
      } else {
        final words = name.split(' ');
        if (words.any((w) => _distance(w, normalized) <= 2)) {
          kind = MatchKind.similarSpelling;
          score = .72;
  ...
  BarcodeResult scanBarcode(Barcode barcode, List<Product> catalog) {
    if (!barcode.isValid) {
      return const BarcodeResult(BarcodeMatchState.invalid, []);
    }
    final key = barcodes[barcode.value];
    final matches = catalog
        .where((p) => p.id == key || (key == 'coffee' && p.tags.contains('coffee')))
        .toList();
  ...
  List<SearchSuggestion> suggestions(List<String> recent) => [
        for (final value in recent.take(3))
          SearchSuggestion(value, SuggestionSource.recent),
        ...
        const SearchSuggestion(
          'Classic Cold Brew',
          SuggestionSource.watchlist,
        ),
        const SearchSuggestion('Avocado Box', SuggestionSource.nearbyDemand),
        const SearchSuggestion('SoundCore', SuggestionSource.popularBrand),
      ];

  @override
  List<RelatedProduct> related(Product product, List<Product> catalog) => catalog
      .where(
        (p) => p.id != product.id &&
            (p.categoryId == product.categoryId || p.brand.id == product.brand.id),
      )
      .take(4)
      .map(
        (p) => RelatedProduct(
