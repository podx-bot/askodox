...
class MockProductDiscoveryRepository implements ProductDiscoveryRepository {
  ...
  List<SmartMatch> matchText(String text, List<Product> catalog) {
    ...
    for (final p in catalog) {
      ...
      final terms = '$name ${p.brand.name.toLowerCase()} ${p.tags.join(' ').toLowerCase()} ${p.categoryId} ${p.subcategoryId}';
      MatchKind? kind;
      double score = 0.0;
      String reason = '';
      ...
      } else if (name == normalized || p.tags.any((e) => e.toLowerCase() == normalized)) {
  ...
  List<SearchSuggestion> suggestions(List<String> recent) => [
        for (final value in recent.take(3)) SearchSuggestion(value, SuggestionSource.recent),
        const SearchSuggestion('Wave Wireless Headphones', SuggestionSource.trending),
        const SearchSuggestion('Classic Cold Brew', SuggestionSource.watchlist),
