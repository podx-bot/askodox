...
class ProductDiscoveryScreen extends ConsumerStatefulWidget {
  ...
  @override
  ConsumerState<ProductDiscoveryScreen> createState() =>
      _ProductDiscoveryScreenState();
}

class _ProductDiscoveryScreenState
    extends ConsumerState<ProductDiscoveryScreen> {
  ...
  List<Widget> _barcode(DiscoveryState state) => [
        TextField(
          controller: barcode,
  ...
  List<Widget> _ocr(DiscoveryState state) => [
        Card(
          child: Padding(
        ...
        if (state.ocrResult != null && state.ocrResult!.matches.isEmpty)
          ...[const ProductRequestActions(compact: true)],
      ];

  List<Widget> _image(DiscoveryState state) => [
        Card(
          child: Padding(
            padding: const EdgeInsets.all(16),
            child: Text(
  ...
  String _title(SearchIntentType mode) => te
      ? switch (mode) {
          SearchIntentType.barcode => 'బార్‌కోడ్ సెర్చ్',
          SearchIntentType.ocr => 'OCR మ్యాచింగ్',
          SearchIntentType.image => 'ఇమేజ్ సెర్చ్',
          SearchIntentType.voice => 'ASKODOX వాయిస్',
          _ => 'స్మార్ట్ సెర్చ్',
        }
      : switch (mode) {
