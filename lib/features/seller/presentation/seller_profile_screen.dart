...
class SellerProfileScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('Shop profile')), body: Center(child: ConstrainedBox(constraints: const BoxConstraints(maxWidth: 700), child: ListView(padding: const EdgeInsets.all(20), children: [
      ...
      const SizedBox(height: 12), OutlinedButton.icon(onPressed: () => context.push('/seller/location'), icon: const Icon(Icons.pin_drop_outlined), label: const Text('Manage shop location')), const SizedBox(height: 8), OutlinedButton.icon(onPressed: () => context.push('/seller/register'), icon: const Icon(Icons.edit_outlined), label: const Text('Edit shop details')), TextButton.icon(onPressed: () { ref.read(sellerProvider.notifier).signOut(); context.go('/seller/login'); }, icon: const Icon(Icons.logout), label: const Text('Sign out')),
    ]))));
  }
}
