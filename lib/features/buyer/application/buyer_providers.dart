...
// Do not pretend a fixed city is the user's current location. Native Android
// location replaces this neutral value when the user taps "Use current location".
final buyerLocationProvider = StateProvider<BuyerLocation>((ref) => const BuyerLocation(
      id: 'current',
      label: 'Current location',
      address: 'Tap to use your device location',
      latitude: 0,
      longitude: 0,
    ));
...
double distanceKm(BuyerLocation a, double latitude, double longitude) {
  if (a.latitude == 0 && a.longitude == 0) return double.infinity;
  const earth = 6371.0;
  final dLat = (latitude - a.latitude) * pi / 180, dLon = (longitude - a.longitude) * pi / 180;
...
List<NearbySellerResult> matchNearby({required List<ProductPriceListing> listings, required BuyerLocation location, required double radiusKm, required SearchFilter filter, required SearchSortOption sort}) {
  ...
  results.sort((a, b) => switch (sort) {
        SearchSortOption.lowestPrice => a.listing.effectivePrice.compareTo(b.listing.effectivePrice),
        SearchSortOption.nearestShop => a.distanceKm.compareTo(b.distanceKm),
        SearchSortOption.highestTrust => b.listing.trustScore.compareTo(a.listing.trustScore),
        SearchSortOption.bestOffer => (b.listing.price - b.listing.effectivePrice).compareTo(a.listing.price - a.listing.effectivePrice),
        SearchSortOption.recentlyUpdated => b.listing.updatedAt.compareTo(a.listing.updatedAt)
      });
  return results;
}
