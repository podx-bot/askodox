...
class DeveloperSimulation {
  const DeveloperSimulation({this.offline = false, this.networkError = false, this.delay = Duration.zero});
  final bool offline, networkError;
  final Duration delay;

  DeveloperSimulation copyWith({bool? offline, bool? networkError, Duration? delay}) => DeveloperSimulation(
...
class DeveloperSimulationNotifier extends Notifier<DeveloperSimulation> {
  ...
  void delay(bool value) => state = state.copyWith(delay: value ? const Duration(seconds: 2) : Duration.zero);
  void reset() => state = const DeveloperSimulation();
}

final developerSimulationProvider = NotifierProvider<DeveloperSimulationNotifier, DeveloperSimulation>(DeveloperSimulationNotifier.new);
...
class DeveloperSettingsScreen extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    if (kReleaseMode) {
      return const Scaffold(body: Center(child: Text('Developer settings are disabled in production.')));
    }

    final sim = ref.watch(developerSimulationProvider);
    final session = ref.watch(authSessionProvider);
    final role = session.role == UserRole.guest ? UserRole.buyer : session.role;
      ...
      body: ListView(
        ...
        children: [
          DropdownButtonFormField<UserRole>(
            ...
            items: [UserRole.buyer, UserRole.seller, UserRole.admin, UserRole.superAdmin]
                .map((item) => DropdownMenuItem(value: item, child: Text(item.name)))
                .toList(),
            onChanged: (value) {
              if (value != null) {
                ref.read(authSessionProvider.notifier).setDemoRole(value);
              }
            },
          ),
          SwitchListTile(
            ...
            onChanged: (value) => ref.read(authSessionProvider.notifier).simulate(value ? AuthStatus.loggedOut : AuthStatus.loggedIn),
          ),
          SwitchListTile(
            title: const Text('Simulate expired session'),
            value: session.status == AuthStatus.sessionExpired,
            onChanged: (value) => ref.read(authSessionProvider.notifier).simulate(value ? AuthStatus.sessionExpired : AuthStatus.loggedIn),
          ),
          SwitchListTile(
            title: const Text('Simulate network error'),
            value: sim.networkError,
            onChanged: ref.read(developerSimulationProvider.notifier).networkError,
          ),
          SwitchListTile(
          ...
          ListTile(
            ...
            onTap: () async {
              ...
              if (context.mounted) {
                ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Mock sync complete')));
              }
            },
          ),
          ListTile(
