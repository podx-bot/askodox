  ...
  Widget _home(bool te) {
    return ListView(
      ...
      children: [
        ...
        Text(
          te ? 'మీకు ఏ విధంగా సహాయం చేయగలను చెప్పండి' : 'How can I help you today?',
          textAlign: TextAlign.center,
          style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w900, color: _ink),
        ),
        const SizedBox(height: 6),
        Text(
          te ? 'మీ అవసరాన్ని టైప్ చేయండి లేదా మాట్లాడండి.' : 'Type or speak what you need.',
          textAlign: TextAlign.center,
        ...
        Row(
          children: [
            Text(te ? 'దగ్గరలో మీ కోసం' : 'Nearby for you', style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: _ink)),
            const Spacer(),
            TextButton(onPressed: () => context.push('/nearby'), child: Text(te ? 'అన్నీ చూడండి' : 'See All')),
          ],
        ),
        SizedBox(
          height: 260,
          child: ListView(
            ...
            children: const [
              _ShopCard(name: 'Sri Lakshmi Chicken Shop', rating: '4.8', distance: '0.5 km', shopId: 'lakshmi-demo'),
              _ShopCard(name: 'Prana Chicken Shop', rating: '4.6', distance: '1.2 km', shopId: 'prana-demo'),
              _ShopCard(name: 'Srinivas Chicken', rating: '4.4', distance: '1.8 km', shopId: 'srinivas-demo'),
            ],
          ),
        ),
      ],
          ...
          children: [
            const Expanded(
              child: Column(
                ...
                children: [
                  Text('Prana Chicken Shop', style: TextStyle(fontSize: 18, fontWeight: FontWeight.w900, color: _ink)),
                  SizedBox(height: 3),
                  Text('Fresh & Hygienic Chicken', style: TextStyle(color: _muted)),
                  SizedBox(height: 9),
                  Chip(label: Text('10% OFF • This Week')),
                ],
              ),
            ),
            Container(
              ...
              child: const Icon(Icons.set_meal_rounded, size: 46, color: Color(0xFFE97A7A)),
            ),
          ],
        ),
      );

  Widget _composer(bool te) {
    return Container(
      padding: EdgeInsets.fromLTRB(10, 8, 10, 8 + MediaQuery.paddingOf(context).bottom),
      decoration: const BoxDecoration(color: Colors.white, border: Border(top: BorderSide(color: Color(0xFFE5EAF2)))),
      child: Row(
        children: [
...
class _ShopCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(18), border: Border.all(color: const Color(0xFFE3E9F2))),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Container(
            height: 105,
            decoration: const BoxDecoration(color: Color(0xFFFFECEC), borderRadius: BorderRadius.vertical(top: Radius.circular(18))),
            child: const Center(child: Icon(Icons.set_meal_rounded, size: 64, color: Color(0xFFE97A7A))),
          ),
          Padding(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(name, maxLines: 2, overflow: TextOverflow.ellipsis, style: const TextStyle(fontWeight: FontWeight.w900, color: _ink)),
                const SizedBox(height: 6),
                Text('★ $rating   •   $distance', style: const TextStyle(color: _muted, fontSize: 12)),
                const SizedBox(height: 6),
                const Wrap(spacing: 5, children: [Chip(label: Text('Fresh')), Chip(label: Text('Hygienic'))]),
                SizedBox(width: double.infinity, child: FilledButton.tonal(onPressed: () => context.push('/shop/$shopId'), child: const Text('View →'))),
              ],
            ),
          ),
        ],
